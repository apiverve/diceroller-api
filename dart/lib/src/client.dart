...
class DicerollerValidationException implements Exception {
  final List<String> errors;

  DicerollerValidationException(this.errors);

  @override
  String toString() => 'DicerollerValidationException: ${errors.join("; ")}';
}
...
/// Dice Roller API client.
///
/// For more information, visit: https://apiverve.com/marketplace/diceroller?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [dice] - Dice notation in XdY format (e.g., 3d6, 2d20, default: 1d6)
...
class DicerollerClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'dice': ValidationRule(type: 'string', required: false),
    'modifier': ValidationRule(type: 'integer', required: false),
  };

  DicerollerClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/diceroller',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DicerollerValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DicerollerValidationException(errors);
    }
  }

  /// Execute a request to the Dice Roller API.
  ///
  /// Parameters are validated before sending the request.
  Future<DicerollerResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DicerollerException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DicerollerResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DicerollerException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DicerollerException('Resource not found');
      } else {
        throw DicerollerException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DicerollerException) rethrow;
      throw DicerollerException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Dice Roller API client.
class DicerollerException implements Exception {
  final String message;

  DicerollerException(this.message);

  @override
  String toString() => 'DicerollerException: $message';
}
