// Package diceroller provides a Go client for the Dice Roller API.
//
// For more information, visit: https://apiverve.com/marketplace/diceroller?utm_source=go&utm_medium=readme
package diceroller
...
// Request contains the parameters for the Dice Roller API.
//
// Parameters:
//   - dice: string - Dice notation in XdY format (e.g., 3d6, 2d20, default: 1d6)
//   - modifier: integer - Modifier to add to the total (can be negative)
type Request struct {
	Dice string `json:"dice,omitempty"` // Optional
	Modifier int `json:"modifier,omitempty"` // Optional
}
...
func (r *Request) Validate() error {
	rules := map[string]ValidationRule{
		"dice": {Type: "string", Required: false},
		"modifier": {Type: "integer", Required: false},
	}
...
// ResponseData contains the data returned by the Dice Roller API.
type ResponseData struct {
	DiceNotation string `json:"dice_notation"`
	NumDice int `json:"num_dice"`
	NumSides int `json:"num_sides"`
	Modifier int `json:"modifier"`
	Rolls []int `json:"rolls"`
	Total int `json:"total"`
	TotalWithModifier int `json:"total_with_modifier"`
	MinRoll int `json:"min_roll"`
	MaxRoll int `json:"max_roll"`
	AverageRoll float64 `json:"average_roll"`
	TheoreticalMin int `json:"theoretical_min"`
	TheoreticalMax int `json:"theoretical_max"`
	TheoreticalAverage float64 `json:"theoretical_average"`
